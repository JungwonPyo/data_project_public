...
class Evaluator(object):
    ...
    def scale_process_rgbX(self, img, modal_x, ori_shape, crop_size, stride_rate, device=None):
        ...
        if new_cols <= crop_size[1] or new_rows <= crop_size[0]:
            ...
            img_pad, margin = pad_image_to_shape(img, stride, cv2.BORDER_CONSTANT, value=0)
            modal_x_pad, margin = pad_image_to_shape(modal_x, stride, cv2.BORDER_CONSTANT, value=0)
            ...
            r_grid = int(np.ceil((pad_rows - stride[0]) / stride[0])) + 1
            c_grid = int(np.ceil((pad_cols - stride[1]) / stride[1])) + 1
            data_scale = torch.zeros(self.class_num, pad_rows, pad_cols).cuda(device)

            for grid_yidx in range(r_grid + 1):
                for grid_xidx in range(c_grid + 1):
                    ...
                    # print('grid_yidx, grid_xidx, s_y, e_y, s_x, e_x: ', grid_yidx, grid_xidx, s_y, e_y, s_x, e_x)
