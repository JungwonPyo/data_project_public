...
def main():
    ...
    if config.TRAIN.RESUME:
        ...
        if os.path.isfile(model_state_file):
            ...
            model.module.model.load_state_dict({k.replace(
                'model.', ''): v for k, v in checkpoint['state_dict'].items() if k.startswith('model.')})
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        if args.local_rank <= 0 and epoch % config.TRAIN.SAVE_MODEL_TERM == 0:
            ...
            # Also, save each save term epoch seperately.
            torch.save(model.module.state_dict(),
                           os.path.join(final_output_dir, 'epoch_%d.pth' % (epoch)))
